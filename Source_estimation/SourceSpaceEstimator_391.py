...
class SourceSpaceEsimator_2(BaseEstimator):
    ...
    def create_source_data(self,raw,fwd,inv,fs=0,mode='mean'):
        ...
        for i in range(len(labels)):
            ...
            if mode=='mean':
                source_alpha= np.mean(abs(apply_inverse_epochs(alpha_raw, inv, lambda2=0.1, method= 'sLORETA', label=label, verbose=False)[0].data),0)
                source_betta= np.mean(abs(apply_inverse_epochs(betta_raw, inv, lambda2=0.1, method= 'sLORETA', label=label, verbose=False)[0].data),0)
