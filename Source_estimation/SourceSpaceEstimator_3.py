...
class SourceSpaceEsimator_2(BaseEstimator):
    ...
    def create_source_data(self,raw,fwd,inv,cluster_ids,labels,fs=0):
        ...
        betta_raw=self.filter_data(raw_matrix,15,25,fs)
        betta_raw=betta_raw.apply_hilbert()

        for i in range(labels):
            label=labels[i]
            source_alpha= abs(apply_inverse_raw(alpha_raw, inv, lambda2=0.01, method= 'MNE', label=label).data)
            source_betta= abs(apply_inverse_raw(betta_raw, inv, lambda2=0.01, method= 'MNE', label=label).data)
